...
class Tree
{
    ...
    {
	    $this->messages[] = "deleting ".$_POST['delete_userID'];
	    $this->deleteID = $_POST['delete_userID'];
	    $this->db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
		if (!$this->db_connection->connect_errno) 
		{
	        $this->user_id = $_SESSION['user_id'];
	        $this->milestone_id = $_POST['milestoneID'];
	        
	        $qry = $this->db_connection->query("
	        	DELETE FROM 
	        		roots 
	        	WHERE 
	        ...
	        if ($qry) {
				...
				if ($qry)
				{
					// delete their gallery etc ?
					$this->messages[] = "Person removed";
				} 	
                
                
            } else {

                $this->errors[] = "Root could not be deleted.";
            }
        } else {
    ...
    public function updateRoot()
    {
	    ...
	    $this->email = $_POST['editRoot_personMail'];
	    $this->name = $_POST['editRoot_personName'];
	    $this->prevRelation = $_POST['editRoot_prevRelation'];
	    
	    $this->fullName = explode(" ", $this->name);
	    $this->firstName = $this->fullName[0];
	    $this->surName = $this->fullName[1];
		...
		if (!$this->db_connection->connect_errno) 
		{
			$qry = $this->db_connection->query("
	        	...
	        	WHERE 
	        		user_id = ".$this->userID."
	        ");
	        //$this->messages[] = "Updated the people db, now for roots";
	        if ($qry) 
	        {
		        // update the status
		        if ($this->relation != $this->prevRelation)
		        {
			        // relation has changed, upate the db
			        $qry = $this->db_connection->query("
			        	UPDATE
			        		roots
			        	SET
			        		family_status = '".$this->relation."'
			        	WHERE
			        		user_id = '".$this->userID."'
			        ");
			        
			        $this->messages[] = "Changes have been saved.";
		        }
		        
                //$this->messages[] = "Details saved";
            } else {
    ...
    {	
        $get_root = $this->db_connection->query("SELECT user_fullname, user_email FROM people WHERE user_id = ".$user_id."");
