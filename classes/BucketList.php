...
{	
    ...
    {
        if (isset($_POST["addBucketList"])) 
        {
            $this->addBucketList();
        }
        if (isset($_POST['bucketlist_delete']))
        {
	        $this->deleteBucketList();
	    }
	    if (isset($_POST['bucketlist_edit']))
	    {
	    ...
	    if (isset($_POST['milestone_delete']))
	    {
    ...
    public function editMileStone()
    {
		...
		if (!$this->db_connection->connect_errno) 
		{
	        ...
	        $this->milestone_text = $_POST['milestone_text'];
	        
	        $query_update_bucketlist = $this->db_connection->query("
	        	UPDATE 
	        		milestones 
	        	SET 
	        		milestone = '".$this->milestone_text."' 
	        	WHERE 
    ...
    private function deleteBucketList()
    {
		...
		if (!$this->db_connection->connect_errno) 
		{
            ...
            $query_delete_milestones = $this->db_connection->query("
	        	DELETE FROM 
	        		milestones 
	        	WHERE 
	        		user_id ='".$this->user_id."' 
	        ...
	        if ($query_delete_milestones) {
		        $this->messages[] = "Milestones removed";
	        }
        } else {
	        $this->errors[] = "Could not connect";
    ...
    private function deleteMileStone()
    {
	    $this->db_connection = new mysqli(DB_HOST, DB_USER, DB_PASS, DB_NAME);
		if (!$this->db_connection->connect_errno) 
		{
	        ...
	        $query_delete_bucketlist = $this->db_connection->query("
	        	DELETE FROM milestones WHERE user_id ='".$this->user_id."' AND milestone_id ='".$this->milestone_id."'
	        ");
	        if ($query_delete_bucketlist) {
	        	//get the file path
	        	...
	        	//delete the file
	        	//unlink(/*file path*/);
                $this->messages[] = "Milestone removed";
            } else {

                $this->errors[] = "Milestone could not be deleted.";
            }
        } else {
	        $this->errors[] = "Could not connect";
